...
const SetApt = () => {
    ...
    const { backgrounds } = useMedia();

    // Initialize with undefined, and then conditionally assign if backgrounds is available
    let leftBgImage = undefined;
    let rightBgImage = undefined;

    // Ensure backgrounds is an array before attempting to find elements
    if (Array.isArray(backgrounds)) {
        leftBgImage = backgrounds.find(bg => bg.id === 'pool39');
        rightBgImage = backgrounds.find(bg => bg.id === 'pool51');
    }
    ...
    return (
        <div className="Set-Apt-container">
            {/* Render background images only if both are found */}
            {leftBgImage && rightBgImage && (
                <>
                    {/* Left background image */}
                    <div
                        className="apt-background-image apt-background-left"
                        style={{ '--bg-image-url': `url(${leftBgImage.src})` }}
                        role="presentation"
                        aria-hidden="true"
                    ></div>
                    {/* Right background image */}
                    <div
                        className="apt-background-image apt-background-right"
                        style={{ '--bg-image-url': `url(${rightBgImage.src})` }}
                        role="presentation"
                        aria-hidden="true"
                    ></div>
                </>

...
const ITEMS_PER_PAGE = 5; // Define the limit for items displayed initially
...
const Profile = () => {
    ...
    if (!authLoading && user && !isOwner && user.email && db) {
      ...
      const unsubscribe = onSnapshot(
        ...
        (querySnapshot) => {
          ...
          // Sort appointments by createdAt in descending order (most recent first)
    ...
    if (!authLoading && user && isOwner && db) {
      ...
      const unsubscribeAppointments = onSnapshot(
        ...
        (querySnapshot) => {
          ...
          appointments.sort((a, b) => b.createdAt.toDate() - a.createdAt.toDate()); // Sort
      ...
      const unsubscribeQuotes = onSnapshot(
        ...
        (querySnapshot) => {
          const quoteRequests = querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
          quoteRequests.sort((a, b) => b.createdAt.toDate() - a.createdAt.toDate()); // Sort
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      ...
      if (phone !== (user.phoneNumber || '')) { // Compare with user's phone if available
         profileUpdates.phone = phone;
      }
      if (address !== (user.address || '')) { // Compare with user's address if available
                  ...
                  {filteredContactSubmissions.length === 0 ? (
                    ...
                    <>
                      ...
                      {filteredContactSubmissions.length > ITEMS_PER_PAGE && (
                        <div className="Profile-showMore-container">
                          <button
                            onClick={() => setShowAllContactSubmissions(!showAllContactSubmissions)}
                            className="Profile-button Profile-showMoreBtn"
                          >
                    ...
                    <div className="Profile-buttons-container">
                      {!editMode ? (
                        ...
                        <>
                          <button
                            type="submit"
                            className="Profile-button Profile-saveBtn"
                          >
                            Save Changes
                          </button>
                          <button
          ...
          <div className="Profile-right-column">
            {selectedAdminItem && (
              <div className="Profile-details-panel Profile-card-base">
                ...
                {selectedAdminItem.email && <p><strong>Email:</strong> {selectedAdminItem.email}</p>}
                {selectedAdminItem.phone && <p><strong>Phone:</strong> {selectedAdminItem.phone}</p>}
                {selectedAdminItem.address && <p><strong>Address:</strong> {selectedAdminItem.address}</p>}
                {selectedAdminItem.date && selectedAdminItem.time && <p><strong>Date & Time:</strong> {selectedAdminItem.date} at {selectedAdminItem.time}</p>}
                {selectedAdminItem.selectedServices && selectedAdminCollection === 'appointments' && (
                  <p><strong>Services:</strong> {selectedAdminItem.selectedServices.join(', ')}</p>
                )}
                {selectedAdminItem.message && <p><strong>Message:</strong> {selectedAdminItem.message}</p>}
                {selectedAdminItem.carMake && <p><strong>Car Make:</strong> {selectedAdminItem.carMake}</p>}
                {selectedAdminItem.carModel && <p><strong>Car Model:</strong> {selectedAdminItem.carModel}</p>}
                {selectedAdminItem.carYear && <p><strong>Car Year:</strong> {selectedAdminItem.carYear}</p>}
  ...
  return (
    <div className="Profile-wrapper">
      <div className="Profile-Dashboard-Dashboard Profile-card-base">
        ...
        <div className="Profile-Dashboard-card">
          <h3 className="Profile-Dashboard-subtitle">Welcome, {user?.displayName || user?.email || 'Customer'}!</h3>
        ...
        <div className="Profile-tabs-container">
          <button
            ...
            onClick={() => {
              setActiveTab('appointments');
              setEditMode(false); // Exit edit mode when switching tabs
              setShowAllCustomerAppointments(false); // Reset show all
            }}
          >
                ...
                {loadingCustomerAppointments ? (
                  ...
                  <>
                    ...
                    {filteredCustomerAppointments.length > ITEMS_PER_PAGE && (
                      <div className="Profile-showMore-container">
                        <button
                          onClick={() => setShowAllCustomerAppointments(!showAllCustomerAppointments)}
                          className="Profile-button Profile-showMoreBtn"
                        >
        ...
        <div className="Profile-right-column">
          {selectedCustomerAppointment && (
            <div className="Profile-details-panel Profile-card-base">
              ...
              {selectedCustomerAppointment.date && <p><strong>Date:</strong> {selectedCustomerAppointment.date}</p>}
              {selectedCustomerAppointment.time && <p><strong>Time:</strong> {selectedCustomerAppointment.time}</p>}
              {selectedCustomerAppointment.selectedServices && (
                <p><strong>Services:</strong> {selectedCustomerAppointment.selectedServices.join(', ')}</p>
              )}
              {selectedCustomerAppointment.address && <p><strong>Address:</strong> {selectedCustomerAppointment.address}</p>}
              {selectedCustomerAppointment.message && <p><strong>Message:</strong> {selectedCustomerAppointment.message}</p>}
              {selectedCustomerAppointment.status && <p><strong>Status:</strong> {selectedCustomerAppointment.status}</p>}
              <button
                onClick={handleCloseDetails}
                className="Profile-button Profile-requestServiceBtn mt-4"
              >
